...
class DreameManager {
        ...
        if (this.mapEnable) {
            ...
            if (adapter.config.enableMiMap) {
                this.mapHelper
                    .login()
                    .then(() => {
                        adapter.log.debug('Dreame map login successful');
                        this.mapReady.login = true;
                    })
                    .catch(error => adapter.log.warn(`Dreame map login failed: ${error.message || error}`));
            }
        }
    ...
    handleMapProperty(element) {
        ...
        if (
            (element.siid === DreameProperties.OBJECT_NAME.siid && element.piid === DreameProperties.OBJECT_NAME.piid) ||
            (isX20MaxModel() && element.siid === 10 && element.piid === 2)
        ) {
            if (typeof element.value === 'string' && element.value.trim()) {
                if (this.mapPointer !== element.value) {
                    adapter.log.debug('Updated Dreame map pointer from OBJECT_NAME');
                }
                this.mapPointer = element.value.trim();
    ...
    async pollMap() {
        ...
        if (!this.mapReady.mappointer) {
            try {
                ...
                if (isX20MaxModel()) {
                    requestProps.push({ did: '', siid: 10, piid: 2 });
                }
