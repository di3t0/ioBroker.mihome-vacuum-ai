...
const DreameActions = {
    ...
    START_SWEEP: { did: '', siid: 2, aiid: 4, control: 'control.startSweep' },
    START_MOP: { did: '', siid: 2, aiid: 5, control: 'control.startMop' },
    START_SWEEP_MOP: { did: '', siid: 2, aiid: 6, control: 'control.startSweepMop' },
    ...
    LOCATE: { did: '', siid: 7, aiid: 1, control: 'control.find' },
...
function isX20MaxModel() {
    return adapter && adapter.config && adapter.config.model === 'xiaomi.vacuum.d109gl';
}
...
class DreameManager {
    constructor(adapterInstance, Miio) {
        ...
        this.mapReady = { login: !adapter.config.enableMiMap, mappointer: false };
        this.mapPollIntervall = Math.max(15000, adapter.config.pingInterval || 20000);
        this.cMapLastPoll = 0;
        this.mapLastSave = 0;
        this.mapSaveIntervall = 30000;
        this.rawPropertyDump = {};
        this.modelSpecificBlockedObjects = [];

        if (isX20MaxModel()) {
            DreameProperties.STATE.siid = 2;
            DreameProperties.STATE.piid = 2;
            DreameProperties.ERROR.control = undefined;
            DreameProperties.CLEANING_TIME.siid = 4;
            DreameProperties.CLEANING_TIME.piid = 3;
            DreameProperties.CLEANED_AREA.siid = 4;
            DreameProperties.CLEANED_AREA.piid = 2;
            DreameProperties.VOLUME.control = undefined;
            DreameProperties.SIDE_BRUSH_LEFT.control = undefined;
            DreameProperties.FILTER_LEFT.control = undefined;
            DreameProperties.SENSOR_DIRTY_LEFT.control = undefined;
            DreameProperties.OBJECT_NAME.siid = 10;
            DreameProperties.OBJECT_NAME.piid = 1;
            DreameProperties.MAP_LIST.siid = 11;
            DreameProperties.MAP_LIST.piid = 1;
            DreameProperties.MULTI_FLOOR_MAP.siid = 11;
            DreameProperties.MULTI_FLOOR_MAP.piid = 1;
            this.modelSpecificBlockedObjects.push(
                'find',
                'sound_volume',
                'sound_volume_test',
                'side_brush',
        ...
        if (this.mapEnable) {
            this.PARAMS.push(DreameProperties.OBJECT_NAME);
            this.PARAMS.push(DreameProperties.MAP_LIST);
            this.PARAMS.push(DreameProperties.MULTI_FLOOR_MAP);
            this.mapHelper = new MapHelper(null, adapter);
            if (adapter.config.enableMiMap) {
                adapter.log.debug('Dreame cloud map login deferred until a valid map pointer is available');
            }
        }
    ...
    isBlockedObject(id) {
        ...
        return DreameBlockedObjects.includes(id) ||
            (Array.isArray(this.modelSpecificBlockedObjects) && this.modelSpecificBlockedObjects.includes(id));
    }

    async writeDebugStates() {
        try {
            await adapter.setStateAsync('deviceInfo.miot_raw', {
                val: JSON.stringify(this.rawPropertyDump, null, 2),
                ack: true,
            });
        } catch (error) {
            adapter.log.debug(`Could not write miot_raw debug state: ${error.message || error}`);
        }
    }
    ...
    async init() {
        ...
        if (adapter.config && adapter.config.model === 'xiaomi.vacuum.d109gl') {
            const explicitStartActions = [
                { _id: 'startSweep', common: { name: 'Start vacuum only', type: 'boolean', role: 'button', read: false, write: true, def: false }, native: {} },
                { _id: 'startMop', common: { name: 'Start mop only', type: 'boolean', role: 'button', read: false, write: true, def: false }, native: {} },
                { _id: 'startSweepMop', common: { name: 'Start vacuum and mop', type: 'boolean', role: 'button', read: false, write: true, def: false }, native: {} },
            ];
            await Promise.all(explicitStartActions.map(async o => {
                const contents = await adapter.setObjectNotExistsAsync(`control.${o._id}`, { type: 'state', common: o.common, native: o.native });
                contents && adapter.log.debug(`Create explicit X20 Max control: ${JSON.stringify(contents)}`);
            }));
        }
        await Promise.all(
        ...
        await adapter.setObjectNotExistsAsync('deviceInfo.miot_raw', {
            type: 'state',
            common: { name: 'Raw MIoT properties (debug)', type: 'string', role: 'json', read: true, write: false, def: '' },
            native: {},
        });
        await adapter.setObjectNotExistsAsync('deviceInfo.miot_last_action', {
            type: 'state',
            common: { name: 'Last MIoT action result (debug)', type: 'string', role: 'json', read: true, write: false, def: '' },
            native: {},
        });
        await Promise.all(
        ...
        if (!this.isBlockedObject('consumable')) {
            ...
            for (let id in objects.stockConsumable.list) {
                ...
                if (!this.isBlockedObject(o.state._id)) {
        ...
        if (this.mapEnable) {
            await Promise.all(
                objects.mapObjects.map(async o => {
                    const contents = await adapter.setObjectNotExistsAsync(`cleanmap${o._id ? `.${o._id}` : ''}`, o);
                    contents && adapter.log.debug(`Create State for Dreame map: ${JSON.stringify(contents)}`);
                }),
            );
        }
    ...
    async getStates() {
        ...
        for (let i = 0; i < this.PARAMS.length; i += chunkSize) {
            ...
            if (DeviceData && DeviceData.result) {
                ...
                answer.forEach(element => {
                    this.rawPropertyDump[`${element.siid}.${element.piid}`] = element;
                    this.handleMapProperty(element);
                    if (isX20MaxModel() && element.siid === 2 && element.piid === 1 && typeof element.value === 'string') {
                        try {
                            const parsed = JSON.parse(element.value);
                            const fw = parsed.base_version || parsed.version || element.value;
                            adapter.setStateAsync('deviceInfo.fw_ver', { val: String(fw), ack: true });
                        } catch (_error) {
                            adapter.setStateAsync('deviceInfo.fw_ver', { val: String(element.value), ack: true });
                        }
                    }
        ...
        await this.writeDebugStates();

        if (this.mapEnable && Date.now() - this.cMapLastPoll > this.mapPollIntervall) {
            await this.pollMap();
        }
    ...
    handleMapProperty(element) {
        if (!this.mapEnable || element.code === -1) {
            return;
        }

        if (element.siid === DreameProperties.OBJECT_NAME.siid && element.piid === DreameProperties.OBJECT_NAME.piid) {
            if (typeof element.value === 'string' && element.value.trim()) {
                let nextPointer = element.value.trim();
                if (isX20MaxModel()) {
                    try {
                        const parsed = JSON.parse(nextPointer);
                        if (parsed && typeof parsed.obj_name === 'string' && parsed.obj_name.trim()) {
                            nextPointer = parsed.obj_name.trim();
                        }
                    } catch (_error) {
                        // keep raw pointer if payload is already the object name
                    }
                }
                if (this.mapPointer !== nextPointer) {
                    adapter.log.debug(`Updated Dreame map pointer from OBJECT_NAME: ${nextPointer}`);
                }
                this.mapPointer = nextPointer;
                this.mapReady.mappointer = Boolean(nextPointer);
            }
            return;
        }

        if (isX20MaxModel() && element.siid === 10 && element.piid === 2) {
            if (typeof element.value === 'string' && element.value.trim()) {
                this.trajectoryPointer = element.value.trim();
            }
            return;
        }

        if (element.siid === DreameProperties.MULTI_FLOOR_MAP.siid && element.piid === DreameProperties.MULTI_FLOOR_MAP.piid) {
            adapter.setStateAsync('cleanmap.mapStatus', { val: element.value ? 3 : 1, ack: true });
            return;
        }

        if (element.siid === DreameProperties.MAP_LIST.siid && element.piid === DreameProperties.MAP_LIST.piid) {
            const mapList = Array.isArray(element.value) ? element.value : [];
            const states = {};
            mapList.forEach((mapEntry, index) => {
                const stateId = mapEntry.map_id || mapEntry.id || index;
                states[stateId] = mapEntry.name || mapEntry.map_name || `map ${index + 1}`;
            });
            if (Object.keys(states).length > 0) {
                adapter.extendObjectAsync('cleanmap.actualMap', { common: { states } });
                adapter.setStateAsync('cleanmap.actualMap', {
                    val: Number(Object.keys(states)[0]) || 0,
                    ack: true,
                });
            }
        }
    ...
    async pollMap() {
        if (!this.mapEnable || !this.mapHelper) {
            return;
        }

        clearTimeout(this.globalTimeouts['getMapData']);

        if (adapter.config.enableMiMap && !this.mapReady.login) {
            try {
                await this.mapHelper.login();
                this.mapReady.login = true;
            } catch (error) {
                adapter.log.debug(`Dreame map login retry failed: ${error.message || error}`);
                return;
            }
        }

        try {
            await this.sendActionToDevice(DreameActions.REQUEST_MAP);
        } catch (error) {
            adapter.log.debug(`REQUEST_MAP failed: ${error.message || error}`);
        }

        if (!this.mapReady.mappointer) {
            try {
                const requestProps = [
                    { did: '', siid: DreameProperties.OBJECT_NAME.siid, piid: DreameProperties.OBJECT_NAME.piid },
                ];
                const response = await this.Miio.sendMessage('get_properties', requestProps);
                if (response && response.result && response.result[0]) {
                    this.handleMapProperty(response.result[0]);
                }
            } catch (error) {
                adapter.log.debug(`OBJECT_NAME refresh failed: ${error.message || error}`);
            }
        }

        if (!this.mapReady.mappointer || !this.mapPointer) {
            return;
        }

        try {
            const data = await this.mapHelper.updateMap(this.mapPointer);
            if (!data) {
                return;
            }

            const dataurl = data[0].toDataURL();
            await adapter.setStateAsync('cleanmap.map64', { val: dataurl, ack: true });
            await adapter.setStateAsync('cleanmap.mapStatus', { val: 3, ack: true });
            this.cMapLastPoll = Date.now();

            if (Date.now() - this.mapLastSave > this.mapSaveIntervall) {
                const buf = data[0].toBuffer();
                adapter.writeFile(
                    `mihome-vacuum-ai.${adapter.instance}.userfiles`,
                    'actualMap.png',
                    buf,
                    error => {
                        if (error) {
                            adapter.log.error('Error by saving of the Dreame map');
                        } else {
                            adapter.setState('cleanmap.mapURL', `/mihome-vacuum-ai.${adapter.instance}.userfiles/actualMap.png`, true);
                        }
            ...
            await adapter.setStateAsync('cleanmap.mapStatus', { val: 0, ack: true });
            adapter.log.warn(`Dreame map update failed: ${error.message || error}`);
        }
    }
    ...
    updateObjectValue(property, control, element) {
        ...
        if (!this.getSpecialHandlingValues(property.control, value)) {
            ...
            if (control === 'deviceInfo.wifi_signal') {
            ...
            if (property.type != undefined) {
                switch (property.type) {
                    case 'int': {
                        const parsed = parseNumericValue(value);
                        value = Number.isFinite(parsed) ? Math.trunc(parsed) : null;
                        break;
                    }
                    case 'boolean':
                        value = Boolean(value);
            ...
            if (control && control.startsWith('consumable.') && Number.isFinite(value)) {
                if (value < 0 || value > 100) {
                    adapter.log.debug(`Suppress implausible consumable value for ${control}: ${value}`);
                    value = null;
                }
            }

            if (value === null || Number.isNaN(value)) {
                adapter.log.debug(`Skip invalid property (${control}) value.`);
                return;
            }
            ...
            for (let action in DreameActions) {
                ...
                if (actionDefinition.control != undefined) {
                    if (id == actionDefinition.control) {
                        if (id === 'control.start' && adapter.config && adapter.config.model === 'xiaomi.vacuum.d109gl') {
                            actionDefinition = DreameActions.START_SWEEP;
                        }
    ...
    async sendActionToDevice(actionDefinition, parameters = '[]') {
        ...
        this.lastActionResult = JSON.stringify(returnData || {}, null, 2);
        await adapter.setStateAsync('deviceInfo.miot_last_action', { val: this.lastActionResult, ack: true });
        const resultCode = returnData && returnData.result ? returnData.result.code : undefined;
        if (resultCode === -1) {
