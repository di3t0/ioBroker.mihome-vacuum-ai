    ...
    async loginStep2() {
        ...
        try {
            ...
            if (data.captchaUrl) {
                ...
                try {
                    const captchaResponse = await this.session.get(captchaUrl, {
                        headers: {
                            'User-Agent': this.agent,
                            Cookie: `${this.commonCookies} pass_ua=web; uLocale=de_DE;`,
                        },
                ...
                if (this.captCode === true) {
                    this.logger.error('CloudApi-Login failed, captcha was provided but Xiaomi requested a new challenge');
                    return { err: 'Login failed, captcha invalid or expired', captchaUrl, captchaImage };
                }
