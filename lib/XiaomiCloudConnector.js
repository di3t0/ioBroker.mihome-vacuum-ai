...
class XiaomiCloudConnector {
    ...
    async loginStep2() {
        ...
        try {
            ...
            this.logger.debug(`CloudApi-Step 2 response: ${JSON.stringify({ code: data.code, description: data.description, captchaUrl: data.captchaUrl, notificationUrl: data.notificationUrl })}`);
            if (data.captchaUrl) {
                ...
                try {
                    const captchaResponse = await this.session.get(`${captchaUrl}${captchaUrl.includes('?') ? '&' : '?'}_dc=${Date.now()}`, {
                        headers: {
                            'User-Agent': this.agent,
                    ...
                    captchaImage = `data:${contentType};base64,${Buffer.from(captchaResponse.data).toString('base64')}`;
                } catch (captchaErr) {
                    this.logger.warn(`CloudApi-Could not download captcha image in session: ${captchaErr.message}`);
                }
                if (this.captCode === true) {
                    this.captCode = false;
                    this.logger.error(`CloudApi-Login failed, captcha was provided but Xiaomi requested a new challenge (${data.description || 'no description'})`);
                    return {
                        err: 'Login failed, captcha invalid or expired',
                        captchaUrl,
                        captchaImage,
                ...
                this.logger.warn(`CloudApi-Login requires captcha: ${captchaUrl}`);
                return {
        ...
        try {
            ...
            if (response.status === 200) {
                ...
                if (!decrypted || typeof decrypted !== 'string') {
                    this.logger.warn('CloudApi-get returned empty decrypted payload');
                    return null;
                }
                try {
                    return JSON.parse(decrypted);
                } catch (parseError) {
                    this.logger.warn(`CloudApi JSON parse failed: ${parseError.message}`);
                    return null;
                }
            }
        } catch (err) {
