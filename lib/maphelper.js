...
function isRRMapBuffer(buffer) {
    return buffer && buffer.length > 2 && buffer[0x00] === 0x72 && buffer[0x01] === 0x72;
}
...
class MapHelper {
                ...
                .then(response => {
                    ...
                    try {
                        if (buffer[0x00] === 0x1f && buffer[0x01] === 0x8b) {
                            const decoded = tryGunzip(buffer);
                            if (!decoded) {
                                reject(new Error('Unable to gunzip map payload'));
                                return;
                            }
                        ...
                        if (isRRMapBuffer(buffer)) {
                            resolve(RRMapParser.PARSEDATA(buffer));
                            return;
                        }

                        const utf8String = buffer.toString('utf8').trim();
                        if (utf8String.startsWith('{') || utf8String.startsWith('[')) {
                            resolve(JSON.parse(utf8String));
                            return;
                        }

                        const base64Payload = utf8String.replace(/^"|"$/g, '').trim();
                        if (base64Payload.length > 16 && /^[A-Za-z0-9+/=_-]+$/.test(base64Payload)) {
                            const decodedBase64 = Buffer.from(base64Payload, 'base64');

                            if (isRRMapBuffer(decodedBase64)) {
                                resolve(RRMapParser.PARSEDATA(decodedBase64));
                                return;
                            }

                            const inflated = tryInflate(decodedBase64) || tryGunzip(decodedBase64);
                            if (inflated) {
                                if (isRRMapBuffer(inflated)) {
                                    resolve(RRMapParser.PARSEDATA(inflated));
                                    return;
                                }
                                const inflatedUtf8 = inflated.toString('utf8').trim();
                                if (inflatedUtf8.startsWith('{') || inflatedUtf8.startsWith('[')) {
                                    resolve(JSON.parse(inflatedUtf8));
                                    return;
                                }
                            }
                        }
                        ...
                        reject(
                            new Error(
                                'Unsupported map payload format. The device may use an encrypted Dreame/Xiaomi map payload that this adapter cannot decode yet.',
                            ),
                        );
                    } catch (e) {
                        reject(e);
    ...
    login() {
        if (!this.config.username || !this.config.password) {
            return Promise.reject(new Error('Xiaomi cloud credentials missing'));
        }
        ...
        return new Promise((resolve, reject) => {
            ...
            if (this.config.mimap === true) {
                ...
                if (!mapurl || typeof mapurl !== 'string' || !mapurl.trim()) {
                    return reject(new Error('Missing map object name'));
                }
                    ...
                    this.getMapURL(mapurl)
                        .then(result => {
                            if (!result || !result.result || !result.result.url) {
                                reject(new Error('Map URL response missing result.url'));
                                return;
                            }
                            mapUrlCache[mapurl] = {
                                expires: result.result.expires_time || 0,
            ...
            this.login()
                .then(() => {
                    ...
                    this.cloudConnector
                        ...
                        .then(json => {
                            try {
                                if (json && json.message === 'ok' && json.result && json.result.url) {
                                    resolve(json);
                                } else if (json && json.message) {
                                    throw new Error(json.message);
                                } else {
                                    throw new Error('Invalid map URL response');
                                }
                            } catch (err) {
